...
public class TropicoCancer extends SpringTest {
	
	@Test@Rollback@Transactional
	public void PaisesTropCanTest() {
		
		Session session = getSession();
		double latTCancer = 23.43722;		
		...
		List<Pais> lista = (List<Pais>) session.createCriteria(Pais.class)
						   .createAlias("capital", "capi")
						   .createAlias("capi.ubicacionGeografica", "ubi")
						   .add(Restrictions.gt("ubi.latitud", latTCancer))
						   .list();
